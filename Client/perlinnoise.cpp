...
PerlinNoise::PerlinNoise(float _persistence, float _frequency, float _amplitude, int _octaves, int _randomseed)
{
  ...
  octaves = _octaves;
  randomseed = _randomseed; //2 + _randomseed * _randomseed;
}
...
float PerlinNoise::GetHeight(float x, float y)
{
  return amplitude * Total(x+10000, y+10000) - Ridges.ridgedMF(x+10000,y+10000);
